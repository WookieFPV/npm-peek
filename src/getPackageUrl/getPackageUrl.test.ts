...
describe("getPackageUrl", () => {
	it.each(validTestData)(
		"return url for %o",
		(_packageName: PackageTestData[0], input: PackageTestData[1], expectedData: PackageTestData[2]) => {
	...
	it.each(invalidTestData)(
		...
		(_packageName: PackageTestData[0], input: PackageTestData[1], _expectedUrl: PackageTestData[2]) => {
			const pkgUrl = getPackageUrl(input);
